...
ACTIVE_PLAN:
- `docs/roadmap/plans/code-block-highlighter-corrections-and-cleanup-plan.md`

CURRENT_SLICE:
- Phase: 0
- Checklist item: Refresh current `main`, reproduce every finding and capture non-recording baselines
- Goal: establish the exact current failure surface before changing scanner code
- Non-goals: no production highlighter changes in this slice

LAST_GOOD_COMMIT:
- `a3f8a00aff344934cba83e3602c03e577f5d1b46`

CURRENT_WORKTREE_STATE:
- Clean / known changes: unknown through the GitHub connector. Record before editing.
...
- result: not started
- known unrelated failures: none recorded. Refresh from the current worktree.
...
NEXT_ACTION:
- refresh `main`, record worktree state and complete Phase 0 reproduction and baseline capture
...
- [ ] Read `AGENTS.md` and this phase's authority documents.
- [ ] Fetch current `main` and record `git rev-parse HEAD`.
- [ ] Record `git status --short --branch` and active worktrees.
- [ ] Confirm whether `code.rs`, its tests, HTML helper docs or benchmark fixture changed after `a3f8a00a`.
- [ ] Update the capsule with the actual starting commit and any path drift.
- [ ] Keep unrelated R5/module-system changes out of this work.

### Reproduce correctness failures

- [ ] Run the focused highlighter tests and record that they currently pass despite the generated corruption.
- [ ] Build the docs release from source.
- [ ] Confirm at least one duplicated import path and one duplicated directive in generated HTML.
- [ ] Confirm the wrong `$code` ownership wording and invalid `String + String` example remain in source docs.
- [ ] Add no correction yet.

### Capture performance and size baselines

- [ ] Run `just bench-validate`.
- [ ] Run `just bench-check` and `just bench-frontend-check` using non-recording modes.
- [ ] Repeat both suites according to the repository optimisation protocol and keep outputs under `/tmp`.
- [ ] Record medians for the dedicated highlighter cases and `docs_check` in the capsule.
- [ ] Record total generated HTML bytes under `docs/release/**` after a clean release rebuild.
- [ ] Do not update benchmark summaries, JSONL history or tracked results.

## Audit and style review

- [ ] Confirm the defect is caused by scanner cursor ownership, not template parsing, whitespace formatting or document-shell indentation.
- [ ] Confirm the planned shared escape owner is `styles/escape_html.rs` and no broader utility already owns the same HTML-project contract.
- [ ] Confirm no production files changed in this phase.
- [ ] Run `git diff --check`.

## Validation gate

- [ ] Run the focused highlighter tests.
- [ ] Run `cargo run --quiet -- tests --case html_code_highlighting --backend html`.
- [ ] Run `cargo run --quiet -- check docs --terse`.
- [ ] Run `just validate` to establish the starting gate.
- [ ] Record exact results in the capsule.
- [ ] Commit only the refreshed plan/capsule if the user-added plan needs a checkpoint.

## Acceptance

- [ ] Current head and worktree state are explicit.
- [ ] Both duplicate-output forms are reproduced from source.
- [ ] Comparable non-recording benchmark and generated-size baselines are recorded.
- [ ] No implementation correction has been mixed into the baseline slice.
...
- [ ] Add `push_escaped_html_text` to `src/projects/html_project/styles/escape_html.rs`.
- [ ] Implement it with safe UTF-8 slice copies between the five ASCII escape bytes.
- [ ] Route the `$escape_html` formatter through this helper.
- [ ] Route code-block plain text and role-span content through this helper.
- [ ] Delete `push_escaped_text` and `push_escaped_char` from `code.rs`.
- [ ] Update `escape_html.rs` file docs to describe both the shared primitive and directive wrapper.
- [ ] Do not move the helper into a broad frontend or project utility module.

### Centralize highlighted-run emission

- [ ] Add one `emit_highlighted_range` scanner method with explicit start/end invariants.
- [ ] Refactor comments, quoted runs, delimiters, directives, paths, numbers and operators to compute local end indexes without mutating `self.index` first.
- [ ] Route every highlighted range through the shared method.
- [ ] Remove repeated flush/span/index/plain-start blocks from token helpers.
- [ ] Keep context-state transitions adjacent to the token semantics, not hidden inside the generic emitter.

### Batch plain runs correctly

- [ ] Refactor word scanning to compute the word range first.
- [ ] If the word has a role, emit it through the shared range method.
- [ ] If the word has no role, advance `index` and leave `plain_start` unchanged.
- [ ] Confirm ordinary identifiers, punctuation and whitespace can remain in one plain run until the next span or EOF.
- [ ] Preserve attached `return!` and `cast!` as one keyword range.

### Fix duplicate output

- [ ] Make directive and path end scanners use local indexes or `consume_while`.
- [ ] Confirm `$md`, `$slot`, `$children`, `@core/io` and `@web/canvas` each emit once.
- [ ] Do not special-case known names. Keep registry-extensible directive spelling and tolerant paths.

### Strengthen tests before accepting

- [ ] Add exact-output tests for one directive and one path.
- [ ] Add a table-driven source-preservation test: strip known role tags and compare with independently escaped source.
- [ ] Include plain identifiers, Unicode, comments, strings, directives, paths and compound operators in that table.
- [ ] Add exact `$escape_html` coverage for all five special characters plus Unicode.
- [ ] Update the existing artifact case with exact-once directive/path assertions and forbidden duplicate patterns.
- [ ] Confirm the new tests fail on the pre-fix implementation and pass after the refactor.

## Audit and style review

- [ ] Search `code.rs` for repeated `flush_plain` + `push_role_span_escaped` + cursor update sequences. Only the shared emitter should own the complete sequence.
- [ ] Search the HTML project for duplicate five-character escape loops. `$code` and `$escape_html` must share one implementation.
- [ ] Confirm no highlighted helper advances `self.index` before handing the original token start to the emitter.
- [ ] Confirm unhighlighted words do not trigger output writes.
- [ ] Review comments for concise ownership and ordering explanations.
- [ ] Run `cargo fmt --all` and `git diff --check`.

## Validation gate

- [ ] Run focused code highlighter and escape directive tests.
- [ ] Run `cargo run --quiet -- tests --case html_code_highlighting --backend html`.
...
- [ ] The highlighter preserves every escaped source byte exactly once.
- [ ] Directive and path corruption is fixed by one emission owner, not two local patches.
- [ ] Plain words are batched.
- [ ] HTML escaping has one HTML-project owner.
- [ ] No accepted role, operator or compiler-word behavior regressed.
...
- [ ] Replace `pending_word_role` with `ExpectedWordRole { start, role }` or an equivalent exact target.
- [ ] Add a helper returning the next non-horizontal-whitespace byte index.
- [ ] Arm an expected role only when that exact position begins an identifier.
- [ ] Consume it only when `word_start == expected.start`; otherwise clear it.
- [ ] Confirm anonymous declarations and intervening punctuation cannot leak a role.

### Consolidate non-Moth word classification

- [ ] Replace `is_keyword`, `is_type_keyword`, `is_literal_word` and `set_non_moth_lookahead` with one direct local classifier.
- [ ] Return the current role and optional next-identifier role as one small value or enum.
- [ ] Keep static direct matches and no allocation.
- [ ] Give `CodeLanguage::Generic` no language-specific word roles.
- [ ] Preserve the accepted bounded JavaScript, TypeScript, Python, Rust and shell role vocabulary only.
- [ ] Do not broaden non-Moth language coverage in this slice.

### Bound Moth function declarations

- [ ] Add explicit paired-pipe context to the Moth scanner state.
- [ ] Toggle it on each structural single `|` delimiter.
- [ ] Classify an ordinary lower-case identifier before `|` as Function only when outside a pipe group.
- [ ] Keep ordinary call and method names before `(` as Function.
- [ ] Preserve priority: compiler words, contracts and PascalCase nominals win before function fallback.
- [ ] Confirm `render |value|` marks only `render` as Function.
- [ ] Confirm `if option is |value|` leaves `value` plain.
- [ ] Confirm untyped function parameters inside `|...|` remain plain.

### Bound Moth contract lists

- [ ] Replace the current broad contract expectation with explicit `ContractListKind` and `ContractState` or an equally clear model.
- [ ] Track generic declaration context after `type` until the declaration's structural boundary.
- [ ] Arm generic contract expectation on `is` only in that context.
- [ ] Keep `must` and `must not` conformance/incompatibility handling.
- [ ] Make `and` continue the current contract kind.
- [ ] Make comma continue conformance lists but end a generic bound before the next generic parameter.
- [ ] Retain ALL_CAPS-followed-by-`must` trait declaration detection.
- [ ] Reset state at newline and declaration boundaries without leaking into later source.
- [ ] Confirm `value is MAX_SIZE` does not mark `MAX_SIZE` as Contract.
- [ ] Confirm `render type Item is DISPLAY_TEXT |...|` does.
- [ ] Confirm `type A is FIRST, B is SECOND` treats `B` as a nominal generic parameter rather than a contract.
- [ ] Confirm `Label must FIRST, SECOND` marks both contracts.

### Tighten Moth path start boundaries

- [ ] Require `@path` to start at a lexical boundary rather than immediately after another `@` or path/identifier continuation.
- [ ] Keep tolerant path continuation and no path validation.
- [ ] Make `@@name` render as two separate `@` operator spans followed by plain `name`.
- [ ] Preserve imports, resource paths and Markdown-style paths in normal boundary positions.

### Add focused regressions

- [ ] Add exact anonymous JavaScript function output.
- [ ] Add interrupted declaration cases for delimiter, comment and newline boundaries.
- [ ] Add Generic profile word-vocabulary absence coverage.
- [ ] Add Moth declaration/capture pipe cases.
- [ ] Add generic-bound, ordinary-`is`, conformance comma and generic comma cases.
- [ ] Add exact `@@name` output.
- [ ] Extend the existing integration fixture rather than adding another case.

## Audit and style review

- [ ] Confirm contextual state fields describe actual lexical facts rather than vague pending booleans.
- [ ] Confirm exact-position expectation removes the need for scattered pending-role resets.
- [ ] Confirm one non-Moth classifier owns word role decisions.
- [ ] Confirm generic and conformance comma behavior is explicit in names and tests.
- [ ] Confirm malformed snippets remain tolerated and cannot panic.
- [ ] Keep `code.rs` within one coherent owner. Do not split it unless the refactor demonstrably becomes harder to review.
- [ ] Remove superseded state variants, helpers and comments.
- [ ] Run `cargo fmt --all` and `git diff --check`.
...
- [ ] No role targets an unspecified later word.
- [ ] Generic blocks remain vocabulary-neutral.
- [ ] Moth captures and parameters are not function declarations.
- [ ] Ordinary `is` comparisons do not create trait colours.
- [ ] Generic and conformance lists classify their commas correctly.
- [ ] Invalid double-`@` input is not presented as a valid path.
...
- [ ] Update `html-helpers.mtf` to state that `$code` is an HTML-builder style directive executed through the frontend formatter registry.
- [ ] Remove the claim that it is unconditionally frontend-owned or available under every builder.
- [ ] Replace `return "Hello, " + name` with valid template concatenation such as `return ["Hello, ", name]`.
- [ ] Keep Basic wording terse and consistent with the advanced owner statement.
- [ ] Update `styles/mod.rs` and `code.rs` file docs where they omit or misname the HTML-project owner.
- [ ] Do not change unrelated theme colours, inline-code margins or font weight.

### Review status documentation

- [ ] Review the existing `Templates and style directives` progress row after stronger tests land.
- [ ] Do not add a new row.
- [ ] Leave the row unchanged if support and coverage wording remain accurate.
- [ ] Make no roadmap edit in this plan.
- [ ] Do not restore or link the original completed implementation plan.

### Rebuild and inspect release docs

- [ ] Run the documentation release build from source.
- [ ] Do not edit generated HTML manually.
- [ ] Search every route for duplicated path/directive adjacency.
- [ ] Search for `moth-code-struct` and `moth-code-parenthesis`.
- [ ] Search for the invalid string-addition example.
- [ ] Inspect aliases, templates, packages/builder/html and progress routes in full context.
- [ ] Confirm directives, paths, functions, contracts and plain source now appear once and in order.

### Recompare output size and performance

- [ ] Record corrected total generated HTML bytes using the same Phase 0 method.
- [ ] Separate the effect of removed duplicate text from intentional role-span overhead.
- [ ] Run `just bench-check` and `just bench-frontend-check` in non-recording mode.
- [ ] Repeat according to the repository optimisation protocol.
- [ ] Compare dedicated highlighter and docs medians with Phase 0.
- [ ] Profile only if a repeatable regression appears.
- [ ] Do not update tracked benchmark history or summaries.
- [ ] Record concise evidence in the capsule and final handoff.

## Audit and style review

- [ ] Verify every changed user example is valid current Moth or explicitly labelled invalid.
- [ ] Verify docs name HTML builder, frontend and formatter ownership accurately.
- [ ] Confirm generated changes derive only from source/code corrections.
- [ ] Confirm unrelated aesthetic CSS is untouched.
- [ ] Confirm progress-matrix and roadmap no-edit decisions are explicit.
- [ ] Run `cargo fmt --all` if Rust changed during corrections and `git diff --check`.

## Validation gate

- [ ] Run focused unit tests.
- [ ] Run the primary integration case.
- [ ] Run `cargo run --quiet -- tests --audit`.
- [ ] Run `cargo run --quiet -- build docs --release`.
- [ ] Run the generated-output duplicate searches.
- [ ] Run `just validate`.
- [ ] Record results in the capsule, commit the accepted slice and set Phase 4 as next.

## Acceptance

- [ ] User docs describe the real owner and show valid syntax.
- [ ] Generated docs contain no known duplicated path/directive output.
- [ ] Generated-size evidence is corrected.
- [ ] Performance is improved or shows no measurable regression.
...
- [ ] Re-read `AGENTS.md`, style, testing and validation authorities.
- [ ] Review the complete diff from the Phase 0 anchor.
- [ ] Confirm one compiler-owned Moth word classifier remains.
- [ ] Confirm one HTML-project escape writer remains.
- [ ] Confirm one highlighted-range emission primitive owns flush/span/cursor updates.
- [ ] Confirm plain words stay batched.
- [ ] Confirm no `pending_word_role`, duplicate word predicates or old emission paths remain.
- [ ] Confirm contextual state is bounded, named and covered by tests.
- [ ] Confirm no new parser, tokenizer, regex, map or allocation-heavy path was added.
- [ ] Confirm old CSS role names remain absent.
- [ ] Confirm no dead helpers, stale comments or broad compatibility wrappers remain.

### Test and output audit

- [ ] Confirm the source-preservation invariant covers representative highlighted and plain runs.
- [ ] Confirm exact-output tests own narrow scanner facts and the integration case owns user-visible HTML.
- [ ] Confirm no redundant new fixture was added.
- [ ] Re-run generated duplicate searches after the final rebuild.
- [ ] Inspect the integration artifact and representative docs routes manually.

### Documentation and status audit

- [ ] Confirm HTML helper docs, file-level docs and progress wording agree.
- [ ] Confirm deferred template-body-aware highlighting remains documented as deferred.
- [ ] Confirm this plan contains the final accepted commits and validation state.
- [ ] Make no roadmap edit.

## Style review

- [ ] Review naming, function size, vertical spacing and comments across every touched Rust file.
- [ ] Prefer explicit local state and direct matches over clever combinators.
- [ ] Confirm extraction into `escape_html.rs` reduced duplication without creating a broad utility.
- [ ] Remove any test-only production API that is no longer justified.
- [ ] Run `cargo fmt --all --check` and `git diff --check`.

## Final validation gate

- [ ] Run focused highlighter and escaping tests.
- [ ] Run complete workspace tests.
- [ ] Run `cargo run --quiet -- tests --audit`.
...
- [ ] Run `just bench-validate`.
- [ ] Run `just validate`.
- [ ] Confirm the worktree is clean after the accepted final commit.

## Final acceptance

- [ ] Every escaped source byte appears exactly once in highlighted output.
- [ ] Directives and paths are not duplicated anywhere in tracked docs.
- [ ] Contextual roles have exact, bounded targets.
- [ ] Scanner and escaping duplication is removed.
- [ ] Tests protect the root invariants rather than only substring presence.
- [ ] Documentation and status tracking are accurate.
