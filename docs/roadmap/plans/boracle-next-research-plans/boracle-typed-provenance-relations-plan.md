...
Status: historical, implemented and retired

This is the archived proposal for package 1. The implementation is complete and the active plan it
was copied to has been deleted with its roadmap entry, so the path below no longer exists and this
file is not a work source. Do not activate it. `README.md` in this directory records the result,
the checkpoint sequence and the accepted deferrals, and
`docs/src/developer-docs/memory-management/boracle/boracle-reference-solver.mtf` is the permanent
authority for the behaviour that shipped. The status, slice, blocker and next-action lines that
drove the work are kept below only as a record of what was proposed.

Original status: proposed
Original current slice: not started
Original blockers: the existing Boracle semantic integration baseline must remain green after the active branch is re-anchored to current `main`
Original next action: activate this plan, establish the working baseline in untracked notes and inventory every current `related_origins`, unknown-provenance and experiment-selection consumer

Original repository path, now deleted:
...
- [ ] Rebase or merge current `main` according to the branch policy.
- [ ] Record the activation commit and clean baseline in untracked working notes.
- [ ] Run the complete existing Boracle lane before edits.
- [ ] Inventory every producer and consumer of:
  - [ ] `related_origins`
  - [ ] `origins_overlap`
  - [ ] `OriginKind`
  - [ ] `CallResultProvenance`
  - [ ] top-like unknown origins
  - [ ] mixed origin traces
  - [ ] copy independence
  - [ ] projection and aggregate child relationships
  - [ ] `BoracleExperiment`
  - [ ] rule-set identity in reports and CLI output
- [ ] Classify each current relationship as identity, directional derivation, possible overlap, proven disjointness or uncertainty.
- [ ] Identify every fallback that creates a result origin when input provenance is empty.
- [ ] Add reduced regression tests for any fallback whose safety depends on an undocumented invariant.
- [ ] Confirm `BorrowProblem::validate` either enforces those invariants or leaves them as explicit top-like facts.
- [ ] Update the small status block with the active slice, blockers and next action only.

### Phase gate

- [ ] Audit the inventory against `problem/`, `last_use/`, `boracle/`, source service and direct docs.
- [ ] Review names, comments and module ownership against the style guide.
- [ ] Run `just boracle`.
- [ ] Run `git diff --check`.
- [ ] Commit the phase as one inventory and regression checkpoint.
