...
Blockers: the existing Boracle semantic integration baseline must remain green after the active branch is re-anchored to current `main`

Next action: activate this plan, establish the working baseline in untracked notes and inventory every current `related_origins`, unknown-provenance and experiment-selection consumer
