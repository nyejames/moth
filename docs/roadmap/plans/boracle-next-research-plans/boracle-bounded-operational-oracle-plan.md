...
This is the archived proposal for package 2. The implementation is complete and the active plan it
...
`docs/src/developer-docs/memory-management/boracle/boracle-operational-oracle.mtf` is the permanent
...
Original blockers: typed provenance overlap evidence and explicit unknown reasons must be available so static and operational disagreements can be classified without guessing

Original next action: activate after the relation foundation is complete, establish the current semantic corpus baseline and specify the smallest executable normalized semantics
