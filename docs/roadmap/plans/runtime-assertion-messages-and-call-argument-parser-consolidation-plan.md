...
STATUS: active (POST_PHASE3_EXTRA_AUDIT_1 accepted; POST_PHASE3_EXTRA_AUDIT_2 pending)
CURRENT_SCOPE: checkpoint the accepted first extra audit cycle, then run the second requested fresh audit-and-correction cycle before Phase 4 work
...
```text
REVISION: Phase 3 checkpoint a7da46ac9 plus the POST_PHASE3_EXTRA_AUDIT_1 correction in the working tree
...
ASSERTION_CASES: 25 cases selected by --tag assert on HTML, including the shared-call diagnostics, runtime local/function/optional/template/cast cases, lazy-success chronology and terminality cases; separate keyword-shadow cases are assert_keyword_shadow_header_error and assert_keyword_shadow_body_error; the separate Wasm boundary is assert_template_message_snapshot
