...
The implementation relationship is now complete for the reference borrow semantics. Boracle
remains feature-gated and the Alpha checker remains the normal compiler authority while this plan
finishes its mandatory independent review and closeout gates.
...
STATUS: active
CURRENT_SLICE: final-audit coverage correction complete; rerun final independent audit and closeout gates
COMPLETED: rebased onto current main; added the red-first source corpus; connected parameter origins, source alias loan derivation, provenance-derived loans, origin-aware overlap, conservative unknown calls, use-driven liveness, exact call-argument events, strict granular-call and call-result validation, origin/loan last-use reporting, stored-child projection provenance through distinct HIR destinations, aggregate generation replacement, generation-aware projection loans, edge-specific scope exits, ancestor-visible terminal/edge kills, duplicate-successor normalization, terminal CFG validation, CFG-fixed-point alias-only write-through versus slot-backed replacement semantics, typed regressions for every semantic-gate source row, exact after-event optional-transfer coverage, bounded generated copy/rebind semantic properties, deterministic metamorphic properties with reduced failure reports, classified Alpha-shared-subset differential cases, slot-backed alias-valued rebind preservation, write-through origin last-use observations, external project-test ownership for the source corpus, permanent authority/progress updates, real-source generated-call boundary coverage, real-source success/error fallible provenance and loan-liveness coverage, and a focused conservative generated-summary fallback regression
NEXT_ACTION: run `just boracle`, `just validate`, `git diff --check`, and the documentation release build; then rerun the mandatory final independent audit through the Ollama GLM route, resolve any findings, and delete this plan with its roadmap entry in the completion commit
VALIDATION: worker slice passed `cargo fmt --all`, 30 Boracle source tests, and the focused generated-fallback problem test. After accepting the correction, `cargo fmt --all`, `just boracle` (30 normalized-problem tests, 5 last-use tests, 52 Boracle/source tests), `just validate` (native Clippy, feature lanes, source audit, 4538 workspace tests, 807 xtask tests, 1897 integration cases, docs check, benchmark sanity, scaling and timer-erasure), `cargo run --quiet -- build docs --release` (72 output files), and `git diff --check` all passed.
AUDITS: Pass 1 interim audit found five accepted issues and their corrections are applied. Pass 2 found six required issues, all corrected. The correction verification audit found four required issues: distinct-destination stored-child propagation, projection-generation kills/deduplication, complete call/result validation and truthful same-call witnesses. Pass 3 found two more: stale aggregate child generations and detached CallResult ownership. Pass 4 found four more: ancestor scope exits, duplicate successors, terminal CFG validation, and weak source aggregate assertions. Pass 5 found the alias-only write-through/slot-rebind blocker and typed corpus gaps; the write-through correction and typed/generated regressions are implemented and pass focused and full gates. Pass 6 was attempted through the configured auditor route but was unavailable because the provider transport reported an invalid peer certificate; no audit edits occurred. The implementation worker route was retried with an isolated temporary state directory and was unavailable for the same provider certificate error; no delegated edits occurred. The mandatory final auditor was attempted against committed head `db08ba16e` with an isolated temporary state directory and was unavailable for the same certificate error; no audit handoff or edits occurred. The resumed fallback final-auditor attempt reached a second provider but ended in a launcher contract violation because its nested sandbox could not inspect the workspace; no audit handoff or edits occurred. Phase 6 semantic-property work was completed coordinator-owned because configured worker providers were unavailable. The fresh final audit against `8890443f3` produced three required findings: slot-backed alias-valued rebind mode preservation, write-through origin last-use observations and external project-test ownership. All three corrections and their focused regressions are applied. The second final audit against `22077ceb1` found one remaining required issue: write-through definitions were excluded from derived loan holder uses. That correction and its real-source liveness regression are now applied and pass both full gates. The relocation worker completed its scoped move and passed focused tests, but the launcher recorded a workspace conflict because concurrent coordinator semantic edits were present; the actual scoped diff was inspected and accepted. The fresh mandatory final audit against `3d05d4fc3` found one medium coverage issue: generated-call boundaries and outcome-sensitive fallible provenance/liveness were not exercised by real-source Boracle tests. The delegated test-only correction added both source cases and the focused HIR fallback test; focused validation passed. The audit routing copy now enables `ollama_glm` as the sole auditor/final-auditor candidate after launcher doctor confirmed the model and server are available.
BLOCKERS: none; focused correction is accepted, full gates and the Ollama GLM final audit remain
NOTES: Boracle remains non-authoritative until the mandatory final audit and completion closeout pass; the latest worker changes are uncommitted and the prior full gates predate them
```
