...
- Status: queued. Language-reference preparation is published. Compiler implementation has not started.
- Current slice: pre-activation documentation preparation, complete except for implementation-coupled updates listed below.
- Blockers: accepted source/token-layout Phase 3, the listed compiler tidy-up and the maintainer-managed additional cleanup must be complete on the implementation tree. The maintainer must release MON implementation after those checkpoints.
- Next action: wait for that release, then refresh the accepted tree and run Phase 0. A documentation merge is not implementation activation.
