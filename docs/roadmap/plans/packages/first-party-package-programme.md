...
BLOCKERS: package development remains paused until accepted data-layout Phase 3 completes. The accepted Phase 2 continuation checkpoint `c17672bb5` (diagnostic correction plus validation-lane stabilization) synchronises the branch; it does not reopen this pause. The merged tree passes workspace tests and all eight feature lanes, but the inherited warning-denied Clippy lane, baseline-equivalent integration failures and the generic-instantiation scaling budget remain open.
...
- JavaScript runtime implementations plus compiler-owned Rust semantic functions for Core constant
  evaluation where the delivered evaluator contract permits them
...
After the compiler-owned Core constant evaluator lands, deterministic Core packages may also own
small Rust semantic implementations used by that evaluator. Those Rust functions and runtime
lowerings implement one Moth contract. They are not a second public package, a backend fallback or
permission to execute JavaScript at compile time. Reuse the delivered typed evaluator owner instead
of creating package-local callback registries.
...
Follow `core-text.md`. The accepted v1 uses Unicode scalar indices consistently across length,
character access, search positions and slicing. Exact search does not normalize or case-fold text.
Runtime JavaScript may use native exact string search, but scalar-position conversion and Unicode
whitespace semantics stay Moth-owned.

Reuse the compiler-owned Rust Text evaluator infrastructure delivered by the prerequisite checkpoint.
...
changing under it; the current pause is the user's scheduling decision and lasts until accepted
data-layout Phase 3 completes. The accepted Phase 2 data-layout continuation checkpoint
synchronises the branch and does not reopen that pause.

### Phase 2 - `@core/text` current v1 slice

Follow `core-text.md`. Implement only the accepted, representable surface after the compiler
foundation checkpoint above has landed.

Mandatory closeout: full phase gate plus the text rich integration scenario and Rust/JS const-eval
parity evidence for every eligible new deterministic operation.
