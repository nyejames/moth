...
- [Compiler source, token and diagnostic data layout](./plans/compiler-source-token-and-diagnostic-data-layout-plan.md) - Phase 1 is complete on main. Continue Phases 2 and 3 on diagnostic-data-layout-changes. Will pause after phase 3 completion and be resumed later on (see below). Its `clippy::result_large_err` removal gates every code-bearing package phase. The Phase 2 checkpoint `34bd000d5` currently fails the all-targets lint build and 851 library tests in its in-flight path-table fork work, and its own timers lane fails to compile on a swapped-argument call, so seven of eight feature lanes fail once that one line is repaired. It therefore blocks a green gate for any branch built on it.
- [First-party Core and Builder package programme](./plans/packages/first-party-package-programme.md) - Phase 0 foundations and Phase 1 workflow activation are merged. Delivered on `packages-and-builder-progress-plan`, adding no package API: the bounded pre-checkpoint hardening slice for the five existing `@core/text` functions and its correction pass; `@core/math` activated ahead of order with its whole existing surface covered and its registration simplified, expansion waiting on a user scope decision; and a `@core/time` design checkpoint that audits the shipped surface and tables its semantic decisions with no implementation accepted. Package slices that need result-slot or const-eval changes stay blocked, while isolated existing-ABI package work may proceed during diagnostics Phases 2/3.
