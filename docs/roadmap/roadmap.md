...
- [First-party Core and Builder package programme](./plans/packages/first-party-package-programme.md) - Phase 0 foundations and Phase 1 workflow activation are merged. A bounded pre-checkpoint hardening slice for the five existing `@core/text` functions is delivered on `packages-and-builder-progress-plan`; it added no package API. Package work runs in parallel with data-layout Phase 2 onward, package slices that need result-slot or const-eval changes stay blocked, while isolated existing-ABI package work may proceed during diagnostics Phases 2/3.
