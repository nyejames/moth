...
- [Compiler source, token and diagnostic data layout](./plans/compiler-source-token-and-diagnostic-data-layout-plan.md) - Phase 1 is complete on main. Phase 2 complete-path interning is accepted at checkpoint `c17672bb5` on `diagnostic-data-layout-changes` (diagnostic correction `e7d9a7ab5` plus validation-lane stabilization). Phase 3 implementation is recorded at checkpoint `6309adf6d`; final exact-checkpoint R5 closeout evidence remains open. The plan pauses after closeout and is explicitly reactivated for Phase 4 later (see below).
