...
- Namespace-qualified type references (`Local as models.Names`, and the same spelling in struct fields) lose their qualification in header ordering hints, so Stage 3 records no local edge and the reference can be resolved before the declaration it names. Pre-existing at `be84e098c`; the fix belongs in header syntax preparation and Stage 3 hint canonicalization. `for_each_named_type_in_parsed_ref` is the lossy walker, and `aliases_waiting_for_constants` in the AST environment classifies constant-dependent aliases through the same walker against bare alias names only, so a qualified dependency can be misclassified there too. Both must be corrected in the same change.
- A module constant whose initializer constructs a generic nominal (`boxed #Box of Int = Box(1)`) fails with `MOTH-INFRA-0001 Parsed call argument retained out-of-range parameter slot 0`. Pre-existing at `be84e098c`, and independent of whether the annotation is written directly or through a type alias. A non-generic constructor in the same position folds correctly, so the fault is in parameter-slot retention for a generic nominal constructor at constant-folding time.
