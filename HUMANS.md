...
Its also an overly long, oppinionated rant about LLMs.
...
Modern agents seem to have a lot of post-training that makes them very "productive" or "proactive". This is good for people who want to leave it alone and come back to find something roughly resembling what they think they wanted on their screen. For more serious projects this relentlessness can cause problems. 

Agents given a high-level broad goal are at high risk of introducing design drift or adding unnessisary code. It's hard to describe our intent in a detailed and bounded way.

Good agent written code is produced by giving it a step-by-step strict implementation plan with all the invariants and rules laid out ahead of time. When it starts or drift, or there is something you've missed, then you stop it and steer it in the new direction rather than let it "proactively" find a way to technically accomplish the broad set of goals or finish the plan at all costs.
...
LLM reviewing, especially when an LLM is loaded up with all the documentation, is quite often very useful and a vital part of orchestrating agents to do work and cut down on how much you will need to notice when staring at that final diff.
...
They keyboard can now be your shrine for carefully considered thought. Not high volume output.

I love writing and have found myself so much more fond of "obviously human" writing in an era where its increasingly obvious who is using these tools to write for them.
...
I don't care how much you tweak or instruct the model to follow certain rules or copy other styles. LLMs will just produce the most statistically sensible version of that thing, and that's just not exciting for humans to read even if it can still be practical and useful. If you whack the temperature up on them, then that chaotic disjointed spontenatity fails to come from a place routed in real experience. It fails to draw those invisible connections that map suprising content to meaning with understated beauty. This art belongs to humans. 

If you think that LLMs are already producing meaningfully creative works with little human guidance, then it might be time to scrutinise the last time you really consumed something that challenged your established aesthetic taste. Often the things we connect with the most also challenge us. They force us to invest effort in order to be engaged. 


### The real clanker slop

Anyone who works on complex projects like these and uses LLMs in that workflow should be acutely aware of how limited they are when it comes to understanding "big picture" design and architecture.
...
The agents reaction to an insight after being adament about a previous position it held usually starts with something along the lines of "you're absolutely right" or "that's an improvement". And to be quite honest, it can be quite a selfishing gratifying ego boost hearing that from a SOTA model. 
...
I tried getting an LLM to write some of the declaritive macros for this, only for it to say something along the lines of: "the formatting you want to do is impossible with declarative macros in Rust and can't be resolved at compile time". I came up with a solution for how those macros needed to parse the context through using a series of layered macros that branched conditionally to one another and got them working. I then shoved it in that stupid LLMs face. I'm sure I got an understated "You're right, this works" in reponse. I wish it could feel humbled for its overconfident assertions about what is actually possible with some human ingenuity.
...
Its important to remember, you need to be able to let go of ideas or recognise bad design as much as you need to be able to push back on the agent when it gets things wrong or doesn't get the vision.
...
If this is an industrial revolution for automated thinking, then its still valuable to remember that "hand-made" things that became mass produced stuck around as the higher quality stuff.

The printer didn't replace layering paint onto a canvas by hand.

The pocket calculator didn't replace the need for mathematicians.

The things we got to do and problems we got to solve just became more abstract and more interesting. Never less important.
