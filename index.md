...
- [builder boundary above frontend](src/build_system/): kw — config, modules, artifacts, cleanup.
    ...
    - [project_config.rs](src/build_system/project_config.rs) + [project_config/](src/build_system/project_config/): locates and reads config.moth, calls the compiler's [config compilation service](src/compiler_frontend/single_source_compilation/config.rs), then validates and applies folded values to `Config` before finalizing its retained source spans.
    ...
    - [create_project_modules](src/build_system/create_project_modules/): Stage 0 module/source discovery.
        - [file_reference_resolution.rs](src/build_system/create_project_modules/file_reference_resolution.rs), [resource_inputs.rs](src/build_system/create_project_modules/resource_inputs.rs): shared directory/synthetic physical resolution for graph-active file-value paths, deterministic settled-target caching, byte-free resource source identity and missing-target watch interests.
