...
#[test]
fn host_io_reads_the_underlying_value_before_logging() {  let mut path_fork = PathInternerFork::empty(); let mut string_table = StringTable::new(); let (type_environment, types) = build_type_environment();
...
#[test]
fn auto_invokes_start_function_when_enabled() {  let mut path_fork = PathInternerFork::empty(); let mut string_table = StringTable::new(); let (type_environment, types) = build_type_environment();
...
let module = build_module(&mut path_fork, &mut string_table, "start_main", vec![block], function, &[]);
