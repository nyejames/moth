...
use super::super::assertions::{
    ...
    extract_executable_scripts, parse_harness_output, run_node_script_within,
...
use super::synthetic_build_results::{VALID_HTML, build_result_with_index_html, success_test_case};
...
fn script_extraction_rejects_an_unterminated_opening_tag() {
    for html in ["<script type=\"module\"", "<p>text</p><script"] {
...
fn script_extraction_rejects_execution_changing_attributes() {
    ...
    for html in [
        ...
        "<script type=\"module\" async>run()</script>",
    ] {
        let message = rejected_script_shape(
            html,
...
fn script_extraction_ignores_tag_case_and_quoted_angle_brackets() {
    ...
    let html = "<SCRIPT TYPE=\"MODULE\" data-note=\"a>b\">run()</SCRIPT>";
