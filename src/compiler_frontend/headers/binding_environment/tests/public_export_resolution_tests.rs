...
//!
//! WHAT: covers module-root-relative effective path derivation for direct-selection dependencies.
//! WHY: the boundary owner must classify same-module dependencies from nested module roots as
//! `NotAPublicExportBoundary` so they fall through to direct source resolution, while
//! cross-module dependencies must still resolve through the target module's public surface.
...
/// A nested module root depending on its own ordinary implementation file via a canonical
...
fn nested_module_root_same_module_dependency_bypasses_public_surface() {
    ...
    assert!(
        ...
        "nested module root depending on its own file should bypass the public surface"
    );
}

/// An entry-root consumer targeting a child module must resolve through that child's
...
/// A cross-module dependency requesting a symbol not exported by the child's public surface
...
fn cross_module_provider_selection_preserves_shell_name_and_diagnostic_path() {
    ...
    assert!(matches!(
        ...
        Some(PublicExportLookupResult::ExportedProviderSelection {
            ...
            diagnostic_path: result_diagnostic_path,
        }) if result_selection == selection
            && result_source_name == source_name
            && result_diagnostic_path == diagnostic_path
    ));
}

/// A deeply nested source-package module root depending on its own ordinary file with a
...
fn source_package_nested_module_root_same_module_dependency_bypasses_public_surface() {
    ...
    assert!(
        ...
        "source-package nested module root depending on its own file should bypass the public surface"
    );
}

/// A consumer with no module root membership should still produce a sensible result
