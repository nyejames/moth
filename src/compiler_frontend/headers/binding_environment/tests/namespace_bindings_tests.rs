...
fn location_for(_path_components: &[&str], _string_table: &mut StringTable) -> SourceSpan {
...
fn test_dependency(header_path: PathId) -> RetainedDependencyClause {
...
fn add_selection(
    ...
    selection_store.push(DependencySelection {
        source_span: SourceSpan::new(SourceId::COMPILATION_ROOT, LocalSpan::source_start()),
...
fn namespace_dependency_default_rejects_keyword_shadow_name_variants() {
    for name in ["config", "Config", "CONFIG", "_config", "_Config"] {
        ...
        let dependency_path =
            intern_path(&["test", &provider_name], &mut string_table, &mut path_fork);
...
fn namespace_dependency_alias_rejects_keyword_shadow_name_variants() {
    for alias in ["config", "Config", "CONFIG", "_config", "_Config"] {
        ...
        let expected_span = dependency
            .namespace_binding_span()
            .copied()
...
#[test]
fn namespace_dependency_alias_collision_retains_exact_alias_span() {
    let mut registry = ExternalPackageRegistry::new();
    register_keyword_test_package(&mut registry, "@test/namespace_alias_collision", "member");

    let mut string_table = StringTable::new();
    let mut path_fork = PathInternerFork::empty();
    let source_file = intern_path(&["src", "@page.moth"], &mut string_table, &mut path_fork);
    let dependency_path = intern_path(
        &["test", "namespace_alias_collision"],
        &mut string_table,
        &mut path_fork,
    );
    let mut dependency = test_dependency(dependency_path);
    ...
    let alias_span = SourceSpan::new(
        SourceId::COMPILATION_ROOT,
        LocalSpan::exact(23, 8, &mut span_builder)
            .expect("focused namespace alias span should fit the inline representation"),
    );
    if let DependencyBindingSyntax::Namespace { alias: Some(alias) } = &mut dependency.binding {
        alias.span = alias_span;
    }
    let expected_span = dependency
        .namespace_binding_span()
        .copied()
    ...
    .expect_err("namespace alias should collide with the same-file declaration");
...
fn explicit_moth_extension_diagnostic_retains_dependency_path_span() {
    ...
    let dependency_span = SourceSpan::new(
        SourceId::COMPILATION_ROOT,
        LocalSpan::exact(17, 11, &mut span_builder)
...
fn selected_dependency_alias_rejects_keyword_shadow_name_variants() {
        ...
        .enumerate()
    {
        ...
        let expected_name = string_table.intern(alias);
        let expected_span = selection_store[0]
            .local_alias()
...
fn provider_declaration_selection_rejects_keyword_shadow_names_before_provider_registration() {
    ...
    for (source_name, local_alias) in cases {
        ...
        let mut path_fork = PathInternerFork::empty();
        let source_file = intern_path(&["src", "@page.moth"], &mut string_table, &mut path_fork);
        let mut dependency = test_dependency(intern_path(
            &["provider"],
            &mut string_table,
            &mut path_fork,
        ));
        ...
        let expected_name = string_table.intern(local_alias.unwrap_or(source_name));
        let expected_span = dependency_selections[0]
            .local_alias()
            .map_or(dependency_selections[0].source_span, |alias| alias.span);
...
fn binding_counters_separate_namespace_clauses_from_selected_names() {
    ...
    module_symbols
        .file_dependency_clauses_by_source
        .insert(source_file, vec![namespace_clause, selection_clause]);
    module_symbols
        .dependency_selections_by_source
        .insert(source_file, selection_store);
...
fn source_receiver_methods_remain_absent_from_namespace_records() {
    let mut string_table = StringTable::new();
    let mut path_fork = PathInternerFork::empty();
    let helper_file = path_fork
        .try_intern_portable_path("src/helper.moth", &mut string_table)
        .expect("test path fits");
    let method_path = path_fork
        .try_intern_portable_path("src/helper/tick", &mut string_table)
        .expect("test path fits");
    let span = location_for(&["src", "@page.moth"], &mut string_table);
    let method_name = path_fork
        .component(method_path)
    ...
    let record = builder
        .build_source_namespace_record(&helper_file, Some(span))
...
#[test]
fn prelude_symbol_visibility_has_no_authored_span() {
    let mut registry = ExternalPackageRegistry::new();
    let package_id = registry
        .register_package(
    ...
    assert!(
        ...
        "compiler-injected prelude symbols must not manufacture authored spans"
    );
}

#[test]
fn explicit_external_symbol_binding_retains_authored_span() {
    let mut registry = ExternalPackageRegistry::new();
    let package_id = registry
        .register_package(
    ...
    let visibility = environment
        ...
        .expect("source file visibility should exist");
    assert_eq!(
        visibility.visible_external_symbol_spans.get(&run_name),
        Some(&dependency_span)
    );
}
...
fn nested_module_root_depends_on_child_facade_resolves_child_root() {
    ...
    let grandchild_mod_file = intern_path(
        &["helper", "child", "@mod.moth"],
        &mut string_table,
        &mut path_fork,
    );
    ...
    let mut module_symbols = ModuleSymbols::empty();
    module_symbols.module_file_paths.insert(helper_mod_file);
    module_symbols.module_file_paths.insert(grandchild_mod_file);
    module_symbols
        .file_module_membership
...
fn reversed_selection_order_keeps_one_provider_surface_identity() {
    ...
    let mut forward = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
    ...
    let mut reverse = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
...
fn differing_evidence_records_with_one_identity_fail_before_projection() {
    ...
    module_symbols.module_file_paths.insert(source_file);
    let mut dependency_selections = Vec::new();
    let mut alpha_dependency =
        test_dependency(intern_path(&["alpha"], &mut string_table, &mut path_fork));
    ...
    let mut beta_dependency =
        test_dependency(intern_path(&["beta"], &mut string_table, &mut path_fork));
    ...
    let error = messages.infrastructure_error().expect(
        "differing evidence records with one identity must fail with an infrastructure error",
    );
    assert!(
        error.msg.contains("evidence identity"),
...
fn provider_selection_public_namespace_member_joins_declaration_surface() {
    ...
    let expected_local_path = path_fork
    ...
    assert_eq!(local_path, &expected_local_path);
    assert_eq!(
        builder
            .environment
...
fn namespace_and_direct_selection_share_provider_semantics() {
    ...
    let mut dependency_selections = Vec::new();
    let mut direct_selection = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
    ...
    let mut namespace = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
...
fn two_aliases_of_one_declaration_retain_one_record() {
    ...
    let mut dependency_selections = Vec::new();
    let mut first = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
    ...
    let mut second = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
...
fn missing_provider_record_fails_deterministically() {
    ...
    let mut missing = test_dependency(intern_path(
        &["provider"],
        &mut string_table,
        &mut path_fork,
    ));
...
fn receiver_methods_reuse_summary_by_origin_storage() {
    ...
    let dependencies = (0..2)
        .map(|index| {
            let mut dependency =
                test_dependency(intern_path(&["shapes"], &mut string_table, &mut path_fork));
            dependency.dependency.dependency_shell_id =
