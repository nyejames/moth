...
/// One resolved resource origin and the optional exact span that first introduced it.
///
/// Generated origins may not have an authored source span. When present, the span retains the
/// source identity that owns the authored bytes.
#[derive(Clone, Debug)]
...
impl ModuleResourceTable {
    ...
    ///
    /// A repeated origin returns the existing handle and keeps the first authored span, so origin
    /// data is stored once however many times the module names the resource.
    ...
    pub(crate) fn intern_origin_with_source(
        ...
        let resource = self.intern_origin(origin.clone(), first_authored_span);
        if self
    ...
    /// Source spans contain no interned string IDs and therefore need no remapping.
