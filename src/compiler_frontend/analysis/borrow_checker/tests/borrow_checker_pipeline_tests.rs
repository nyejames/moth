...
#[test]
fn frontend_check_borrows_propagates_failures() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
let z = symbol("z", &mut path_fork, &mut string_table);
...
#[test]
fn successful_borrow_report_can_be_stored_on_module() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = entry_and_start(&mut path_fork, &mut string_table);
let external_package_registry = default_external_package_registry(&mut string_table);

let counter = symbol("counter", &mut path_fork, &mut string_table);
