...
let item_argument = hir
    ...
    .find_map(|statement| {
        ...
        (hir.side_table.resolve_local_name(*local, &path_fork, &string_table) == Some("card"))
            .then_some(args[0].id)
    })
...
"#;
    ...
    let point_local = hir
        ...
        .find(|local| hir.side_table.resolve_local_name(local.id, &path_fork, &string_table) == Some("point"))
        .map(|local| local.id)
