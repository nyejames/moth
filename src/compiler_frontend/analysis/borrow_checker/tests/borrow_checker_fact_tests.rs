...
#[test]
fn statement_terminator_and_value_facts_are_populated() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn drop_statement_produces_statement_fact() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = entry_and_start(&mut path_fork, &mut string_table);
let external_package_registry = default_external_package_registry(&mut string_table);
...
#[test]
fn statement_entry_state_reflects_last_use_reborrow_window() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = entry_and_start(&mut path_fork, &mut string_table);
let external_package_registry = default_external_package_registry(&mut string_table);
...
let data_local =
    find_local_by_name(&hir, &path_fork, &string_table, "data").expect("should locate the source local");
...
#[test]
fn optional_assignment_transfer_keeps_source_state_and_records_advisory_fact() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = entry_and_start(&mut path_fork, &mut string_table);
let external_package_registry = default_external_package_registry(&mut string_table);
...
let source_local =
    find_local_by_name(&hir, &path_fork, &string_table, "source").expect("should locate the source local");
let target_local =
    find_local_by_name(&hir, &path_fork, &string_table, "target").expect("should locate the target local");
...
let label_statement = find_assign_statement_id_for_local_name(&hir, &path_fork, &string_table, "label")
...
#[test]
fn retained_unknown_result_borrows_possible_final_use_argument() { let mut path_fork = crate::compiler_frontend::symbols::path_interner::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = entry_and_start(&mut path_fork, &mut string_table);
...
let result_assignment = find_assign_statement_id_for_local_name(&hir, &path_fork, &string_table, "result")
...
fn find_local_by_name(
    ...
    hir.blocks
        ...
        .find(|local| hir.side_table.resolve_local_name(local.id, path_fork, string_table) == Some(name))
        .map(|local| local.id)
}
    ...
    for block in &hir.blocks {
        for statement in &block.statements {
            if let HirStatementKind::Assign {
                ...
                && hir.side_table.resolve_local_name(*local, path_fork, string_table) == Some(name)
            {
