...
fn reactive_parameter_summary_retains_subscription_without_transfer() {
    ...
    let (entry_path, start_name) = entry_and_start(&mut path_fork, &mut string_table);
    let external_package_registry = default_external_package_registry(&mut string_table);
    let render_name = symbol(
        "render_reactive_parameter",
        &mut path_fork,
        &mut string_table,
    );
    ...
    let mut parameter = param_with_type_id(parameter_path, builtin_type_ids::INT, false, None);
...
fn reactive_parameter_does_not_grant_mutation_permission() {
    ...
    parameter.value.reactive_source =
        Some(reactive_source(param_path, ReactiveSourceKind::Parameter));
