...
let chooser = super::symbol("choose_match", &mut path_fork, &mut string_table);
...
#[test]
fn non_unit_function_with_exhaustive_choice_match_returns_on_all_arms() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let label_fn_name = super::symbol("label_status", &mut path_fork, &mut string_table);
let status_path = super::symbol("Status", &mut path_fork, &mut string_table);
let status_local = super::symbol("status", &mut path_fork, &mut string_table);
...
#[test]
fn lowers_match_with_literal_arms_and_explicit_default_wildcard() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
#[test]
fn lowers_match_with_guarded_arm_into_hir_guard_expression() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
#[test]
fn match_guard_rejects_lowering_when_guard_emits_prelude_statements() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
let ast = build_ast_with_registered_types(vec![start_fn], entry_path);
let err = lower_ast(ast, &mut string_table, &mut path_fork)
...
#[test]
fn match_rejects_non_literal_pattern_expressions() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
#[test]
fn break_outside_loop_reports_hir_transformation_error() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
...
let ast = build_ast_with_registered_types(vec![start_fn], entry_path);
let err = lower_ast(ast, &mut string_table, &mut path_fork).expect_err("break outside loop should fail");
let error = err
    .infrastructure_error()
    .expect("HIR lowering failure should be wrapped for rendering");
assert_eq!(&error.error_type, &ErrorType::HirTransformation);
...
#[test]
fn continue_outside_loop_reports_hir_transformation_error() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
...
let ast = build_ast_with_registered_types(vec![start_fn], entry_path);
let err = lower_ast(ast, &mut string_table, &mut path_fork).expect_err("continue outside loop should fail");
let error = err
    .infrastructure_error()
    .expect("HIR lowering failure should be wrapped for rendering");
assert_eq!(&error.error_type, &ErrorType::HirTransformation);
...
#[test]
fn top_level_return_reports_hir_transformation_error() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
...
let err = lower_ast(ast, &mut string_table, &mut path_fork).expect_err("top-level return should fail");
...
#[test]
fn unit_implicit_return_lowers_to_return_terminator() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
...
let ast = build_ast_with_registered_types(vec![start_fn], entry_path);
let (module, _type_environment) =
    lower_ast(ast, &mut string_table, &mut path_fork).expect("unit fallthrough should succeed");
let start = &module.functions[module
    .start_function
    .expect("normal test module should have start")
    .0 as usize];
...
#[test]
fn side_table_maps_statement_and_terminator_spans() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
#[test]
fn lowers_relational_pattern_to_hir_relational() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
#[test]
fn lowers_guarded_relational_pattern_preserving_guard_separation() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
#[test]
fn lowers_choice_match_arms_to_hir_choice_variant_patterns() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let status_path = super::symbol("Status", &mut path_fork, &mut string_table);
let ready_name = string_table.intern("Ready");
let busy_name = string_table.intern("Busy");
...
#[test]
fn lowers_option_present_capture_to_payload_assignment() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let maybe_name = super::symbol("maybe", &mut path_fork, &mut string_table);
let capture_name = string_table.intern("value");
let capture_path = super::symbol("value", &mut path_fork, &mut string_table);
