...
fn field_symbol(
    ...
    path_fork
        .try_intern_child(*parent, string_table.intern(field_name))
...
fn runtime_template_range_loop_expression(
    ...
    let aggregate_wrapper =
        text_aggregate_wrapper_node(aggregate_prefix, aggregate_suffix, string_table, &span);
...
#[test]
fn expression_test_builder_produces_valid_hir_module_metadata() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_slot_placeholder_materializes_as_no_output_owned_node() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn escaped_slot_insert_helpers_fail_when_they_reach_hir_runtime_lowering() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn escaped_slot_definition_helpers_fail_when_they_reach_hir_runtime_lowering() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let body_slot = string_table.intern("body");
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn runtime_template_without_handoff_reports_compiler_bug() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn top_level_loop_control_handoff_reports_compiler_bug() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let handoff = OwnedRuntimeTemplateHandoff {
...
#[test]
fn lowers_primitive_literals() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
let string_expr = Expression::string_slice(text, span, ValueMode::ImmutableOwned);
...
#[test]
fn lowers_reference_to_registered_local() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn lowers_reference_to_module_constant_when_local_is_missing() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn lowers_runtime_rpn_arithmetic_stack_correctly() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let x = super::symbol("x", &mut path_fork, &mut string_table);
...
let items = vec![
    ...
    runtime_operator_item(Operator::Multiply, span),
    runtime_operator_item(Operator::Add, span),
];

let expr = runtime_expr(items, builtin_type_ids::INT, span, ValueMode::MutableOwned);
let lowered = builder
    .lower_expression(&expr)
...
#[test]
fn runtime_division_subexpression_infers_float_type_in_hir() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let expected_float = builtin_type_ids::FLOAT;

let items = vec![
    runtime_operand_item(Expression::int(5, span, ValueMode::ImmutableOwned)),
    runtime_operand_item(Expression::int(2, span, ValueMode::ImmutableOwned)),
    runtime_operator_item(Operator::Divide, span),
    runtime_operand_item(Expression::int(1, span, ValueMode::ImmutableOwned)),
    runtime_operator_item(Operator::Add, span),
];
...
#[test]
fn runtime_integer_division_lowers_to_hir_int_div_with_int_type() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
let items = vec![
    runtime_operand_item(Expression::int(5, span, ValueMode::ImmutableOwned)),
    runtime_operand_item(Expression::int(2, span, ValueMode::ImmutableOwned)),
    runtime_operator_item(Operator::IntDivide, span),
];
...
#[test]
fn lowers_unary_not_in_runtime_rpn() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);

let items = vec![
    runtime_operand_item(Expression::bool(true, span, ValueMode::ImmutableOwned)),
    runtime_operator_item(Operator::Not, span),
];

let expr = runtime_expr(items, builtin_type_ids::BOOL, span, ValueMode::MutableOwned);
let lowered = builder
    .lower_expression(&expr)
...
#[test]
fn lowers_range_operator_in_runtime_rpn() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);

let items = vec![
    runtime_operand_item(Expression::int(1, span, ValueMode::ImmutableOwned)),
    runtime_operand_item(Expression::int(9, span, ValueMode::ImmutableOwned)),
    runtime_operator_item(Operator::Range, span),
];
...
#[test]
fn lowers_function_call_to_call_statement_and_temp_load() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn expression_function_call_uses_variant_result_type_ids_for_single_return() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
let expected_int = builder
    .lower_type_id(builtin_type_ids::INT, &span)
...
#[test]
fn expression_function_call_uses_variant_result_type_ids_for_no_return() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn expression_function_call_uses_variant_result_type_ids_for_multi_return() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn expression_host_call_uses_variant_result_type_ids() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn expression_handled_fallible_call_fallback_uses_variant_result_type_ids() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let function_name = super::symbol("handled_result", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let ok_type = builder
    .lower_type_id(builtin_type_ids::INT, &span)
...
let err_type = builder
    .lower_type_id(builtin_type_ids::STRING, &span)
    ...
    FallibleHandling::Handler {
        ...
        body: vec![AstNode {
            kind: NodeKind::ThenValue(ProducedValues {
                expressions: vec![Expression::int(7, span, ValueMode::ImmutableOwned)],
                span,
            }),
            span,
            scope: test_scope,
...
#[test]
fn expression_handled_result_derives_success_slots_from_tuple_type_id() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn lowers_fresh_mutable_call_argument_via_hidden_local_with_origin_metadata() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
let call_expr =
    Expression::function_call_with_arguments(function_name, vec![fresh_argument], vec![], span);
...
assert!(
    origin.call_span.is_none(),
    "synthetic fresh mutable argument should not fabricate source provenance"
); }

#[test]
fn lowers_receiver_method_call_with_receiver_as_first_argument() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let method_path = super::symbol("Vector2/reset", &mut path_fork, &mut string_table);
let receiver_name = super::symbol("vec", &mut path_fork, &mut string_table);
let receiver_struct = super::symbol("Vector2", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn lowers_builtin_scalar_receiver_method_call_with_receiver_as_first_argument() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let method_path = super::symbol("Int/double", &mut path_fork, &mut string_table);
let receiver_name = super::symbol("value", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn lowers_host_call_expression_with_host_target() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn preserves_left_to_right_call_prelude_order_in_nested_call_args() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let first = super::symbol("first", &mut path_fork, &mut string_table);
let second = super::symbol("second", &mut path_fork, &mut string_table);
let outer = super::symbol("outer", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn malformed_runtime_rpn_reports_hir_transformation_error() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);

let expr = runtime_expr(
    vec![runtime_operator_item(Operator::Add, span)],
    builtin_type_ids::INT,
    span,
    ValueMode::MutableOwned,
);
...
#[test]
fn runtime_template_expression_lowers_inline_to_accumulator() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_handoff_expression_lowers_inline_to_accumulator() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let hello = string_table.intern("hello");
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn runtime_template_handoff_expression_flattens_nested_linear_handoff() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let before = string_table.intern("before ");
...
let outer_template = runtime_template_expression(
    span,
    vec![
        Expression::string_slice(before, span, ValueMode::ImmutableOwned),
        inner_template,
        Expression::string_slice(after, span, ValueMode::ImmutableOwned),
    ],
    &string_table,
);
...
#[test]
fn runtime_template_inline_accumulator_coerces_non_string_segments() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);

let expr = runtime_template_expression(
    span,
    vec![Expression::int(5, span, ValueMode::ImmutableOwned)],
    builder.string_table,
);
...
#[test]
fn reactive_linear_template_keeps_subscription_chunks_lazy() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
...
#[test]
fn runtime_template_lowers_nested_templates_in_order() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
let nested = runtime_template_expression(
    span,
    vec![Expression::string_slice(b, span, ValueMode::ImmutableOwned)],
    builder.string_table,
);
...
let expr = runtime_template_expression(
    span,
    vec![
        Expression::string_slice(a, span, ValueMode::ImmutableOwned),
        nested,
        Expression::string_slice(c, span, ValueMode::ImmutableOwned),
    ],
    builder.string_table,
);
...
#[test]
fn runtime_template_control_flow_bool_if_lowers_inline_without_helper_call() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_control_flow_bool_if_branch_preserves_fallible_propagation_cfg() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_control_flow_bool_if_without_else_appends_nothing_on_false_path() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let show_name = super::symbol("show", &mut path_fork, &mut string_table);
let shown = string_table.intern("shown");
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn runtime_template_control_flow_bool_if_coerces_dynamic_branch_chunks() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let show_name = super::symbol("show", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
register_local(
...
#[test]
fn runtime_template_control_flow_option_capture_lowers_match_and_payload_binding() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_control_flow_option_capture_without_else_appends_nothing_when_absent() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let maybe_name = super::symbol("maybe_name", &mut path_fork, &mut string_table);
let capture_name = string_table.intern("name");
let capture_path = super::symbol("name", &mut path_fork, &mut string_table);
let span = None;
...
#[test]
fn runtime_template_control_flow_loop_range_lowers_inline_and_wraps_aggregate_when_emitted() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_control_flow_loop_collection_materializes_iterable_and_length_once() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn runtime_template_control_flow_conditional_loop_rechecks_condition_and_wraps_when_emitted() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let prefix = string_table.intern("<wrap>");
let suffix = string_table.intern("</wrap>");
let tick = string_table.intern("tick");
let span = None;
let keep_going_path = super::symbol("keep_going", &mut path_fork, &mut string_table);
let mut builder = setup_builder(&mut string_table, &mut path_fork);
register_local(
    &mut builder,
...
#[test]
fn runtime_template_control_flow_loop_empty_body_does_not_mark_iteration_emitted() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let prefix = string_table.intern("<wrap>");
let suffix = string_table.intern("</wrap>");
let span = None;
let limit_path = super::symbol("limit", &mut path_fork, &mut string_table);
...
#[test]
fn local_resolution_uses_full_path_identity_not_leaf_name() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let x_leaf = string_table.intern("x");
let scope_a = super::symbol("scope_a", &mut path_fork, &mut string_table);
let scope_b = super::symbol("scope_b", &mut path_fork, &mut string_table);
let local_a = path_fork.try_intern_child(scope_a, x_leaf).expect("test path fits");
let local_b = path_fork.try_intern_child(scope_b, x_leaf).expect("test path fits");
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn nominal_struct_identity_uses_field_parent_path() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let struct_path = super::symbol("MyStruct", &mut path_fork, &mut string_table);
let field_path = field_symbol(&struct_path, "value", &mut path_fork, &mut string_table);
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn rejects_const_record_struct_instance_runtime_lowering() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let struct_path = super::symbol("Palette", &mut path_fork, &mut string_table);
let field_path = field_symbol(&struct_path, "red", &mut path_fork, &mut string_table);
...
let expression = Expression::struct_instance(
    ...
    vec![Declaration {
        id: field_path,
        value: Expression::string_slice(field_value, span, ValueMode::ImmutableOwned),
        binding_span: None,
        config_qualifier: None,
    }],
    span,
...
#[test]
fn temp_locals_are_not_resolvable_as_user_symbols() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let callee = super::symbol("callee", &mut path_fork, &mut string_table);
let temp_name = super::symbol("__hir_tmp_0", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn field_access_uses_base_struct_identity_not_global_leaf_lookup() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let struct_a = super::symbol("StructA", &mut path_fork, &mut string_table);
let struct_b = super::symbol("StructB", &mut path_fork, &mut string_table);
let field_leaf = string_table.intern("value");
let field_a = path_fork.try_intern_child(struct_a, field_leaf).expect("test path fits");
let field_b = path_fork.try_intern_child(struct_b, field_leaf).expect("test path fits");
let local_name = super::symbol("my_struct", &mut path_fork, &mut string_table);
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let int_type = builtin_type_ids::INT;
...
#[test]
fn field_access_from_module_constant_base_materializes_temp_place() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let format_name = super::symbol("format", &mut path_fork, &mut string_table);
let format_struct = super::symbol("Format", &mut path_fork, &mut string_table);
let center_leaf = string_table.intern("center");
let center_field = path_fork.try_intern_child(format_struct, center_leaf).expect("test path fits");
...
#[test]
fn const_record_module_constant_field_access_lowers_field_value_without_struct_construct() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let palette_name = super::symbol("palette", &mut path_fork, &mut string_table);
let palette_struct = super::symbol("Palette", &mut path_fork, &mut string_table);
let red_leaf = string_table.intern("red");
let red_field = path_fork.try_intern_child(palette_struct, red_leaf).expect("test path fits");
...
#[test]
fn lowers_collection_builtin_host_calls_from_explicit_ast_nodes() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let receiver_name = super::symbol("values", &mut path_fork, &mut string_table);
...
let cases = vec![
    ...
    (
        ...
        vec![
            CallArgument::positional(
                Expression::int(0, span, ValueMode::ImmutableOwned),
                CallAccessMode::Shared,
                span,
            ),
            CallArgument::positional(
                Expression::int(99, span, ValueMode::ImmutableOwned),
                CallAccessMode::Shared,
                span,
            ),
        ],
    ...
    (
        ...
        vec![CallArgument::positional(
            Expression::int(4, span, ValueMode::ImmutableOwned),
            CallAccessMode::Shared,
            span,
        )],
        vec![fallible_none_result],
...
#[test]
fn map_literal_lowering_preserves_entry_order() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
...
let expression = Expression::new(
    ExpressionKind::MapLiteral(vec![
        MapLiteralEntry {
            key: Expression::string_slice(priya, span, ValueMode::ImmutableOwned),
            value: Expression::int(10, span, ValueMode::ImmutableOwned),
        },
        MapLiteralEntry {
            key: Expression::string_slice(grace, span, ValueMode::ImmutableOwned),
            value: Expression::int(12, span, ValueMode::ImmutableOwned),
        },
    ]),
    span,
    map_type,
...
#[test]
fn map_builtin_calls_lower_to_first_class_hir_ops() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let scores_name = super::symbol("scores", &mut path_fork, &mut string_table);
...
let key = CallArgument::positional(
    Expression::string_slice(key_name, span, ValueMode::ImmutableOwned),
    CallAccessMode::Shared,
    span,
);
...
#[test]
fn lowers_choice_variant_expression_to_hir_variant_construct() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;

let status_path = super::symbol("Status", &mut path_fork, &mut string_table);
...
#[test]
fn collection_expression_lowering_preserves_fixed_type_identity() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
let expression = Expression::new(
    ExpressionKind::Collection(vec![Expression::int(1, span, ValueMode::ImmutableOwned)]),
    span,
    fixed_collection,
    crate::compiler_frontend::datatypes::DataType::Inferred,
    ValueMode::ImmutableOwned,
...
#[test]
fn lowers_option_none_to_hir_variant_construct() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn lowers_fallible_success_to_hir_variant_construct() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
let value_expr = Expression::int(42, span, ValueMode::ImmutableOwned);
...
#[test]
fn lowers_fallible_error_to_hir_variant_construct() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
let value_expr = Expression::string_slice(error_text, span, ValueMode::ImmutableOwned);
let result_expr = Expression::result_construct(
...
#[test]
fn external_float_call_emits_validate_float_in_current_block() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn external_float_call_in_builtin_error_function_validates_with_return_error() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let function_name = super::symbol("external_float_boundary", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let float_type = builder
    .lower_type_id(builtin_type_ids::FLOAT, &span)
...
#[test]
fn external_int_call_does_not_emit_validate_float() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
...
#[test]
fn external_fallible_float_call_propagation_validates_success() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let function_name = super::symbol("fallible_float", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let float_type = builder
    .lower_type_id(builtin_type_ids::FLOAT, &span)
...
#[test]
fn external_fallible_float_call_catch_validates_success() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let function_name = super::symbol("fallible_float_catch", &mut path_fork, &mut string_table);
let span = None;
let mut builder = setup_builder(&mut string_table, &mut path_fork);
let float_type = builder
    .lower_type_id(builtin_type_ids::FLOAT, &span)
...
let catch_expr = wrap_catch_expression(
    ...
    FallibleHandling::Handler {
        ...
        body: vec![AstNode {
            kind: NodeKind::ThenValue(ProducedValues {
                expressions: vec![Expression::float(0.0, span, ValueMode::ImmutableOwned)],
                span,
            }),
            span,
            scope: function_name,
