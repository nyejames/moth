...
let (module, _type_environment) = lower_ast(build_ast_with_registered_types(vec![can_fail_function, start_function], entry_path), &mut string_table, &mut path_fork)
...
#[test]
fn direct_return_result_propagation_lowers_to_explicit_success_and_error_edges() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_fail_name = super::symbol("can_fail", &mut path_fork, &mut string_table);
let forward_name = super::symbol("forward", &mut path_fork, &mut string_table);
let location = None;
...
let (module, _type_environment) = lower_ast(build_ast_with_registered_types(
...
#[test]
fn direct_return_result_propagation_allows_alias_success_return() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let source_name = super::symbol("source", &mut path_fork, &mut string_table);
let source_input = super::symbol("input", &mut path_fork, &mut string_table);
let forward_name = super::symbol("forward", &mut path_fork, &mut string_table);
let forward_input = super::symbol("input", &mut path_fork, &mut string_table);
let location = None;
...
#[test]
fn declaration_result_propagation_assigns_unwrapped_success_on_success_edge() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_fail_name = super::symbol("can_fail", &mut path_fork, &mut string_table);
let forward_name = super::symbol("forward", &mut path_fork, &mut string_table);
let value_name = path_fork.try_intern_child(forward_name, string_table.intern("value")).expect("test path fits");
let location = None;
...
#[test]
fn multi_bind_result_propagation_projects_tuple_slots_after_success_edge() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let pair_name = super::symbol("pair", &mut path_fork, &mut string_table);
let forward_name = super::symbol("forward", &mut path_fork, &mut string_table);
let first_id = path_fork.try_intern_child(forward_name, string_table.intern("first")).expect("test path fits");
let count_id = path_fork.try_intern_child(forward_name, string_table.intern("count")).expect("test path fits");
let location = None;
...
#[test]
fn call_argument_result_propagation_lowers_before_outer_call() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_fail_name = super::symbol("can_fail", &mut path_fork, &mut string_table);
let consume_name = super::symbol("consume", &mut path_fork, &mut string_table);
let consume_input = path_fork.try_intern_child(consume_name, string_table.intern("input")).expect("test path fits");
let forward_name = super::symbol("forward", &mut path_fork, &mut string_table);
let value_name = path_fork.try_intern_child(forward_name, string_table.intern("value")).expect("test path fits");
let location = None;
...
#[test]
fn runtime_binary_result_propagation_lowers_before_operator() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_fail_name = super::symbol("can_fail", &mut path_fork, &mut string_table);
let forward_name = super::symbol("forward", &mut path_fork, &mut string_table);
let value_name = path_fork.try_intern_child(forward_name, string_table.intern("value")).expect("test path fits");
...
#[test]
fn return_bang_lowers_to_explicit_error_terminator() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_fail_name = super::symbol("can_fail", &mut path_fork, &mut string_table);
let location = None;
...
#[test]
fn fallible_success_return_lowers_to_explicit_success_terminator() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_succeed_name = super::symbol("can_succeed", &mut path_fork, &mut string_table);
let location = None;
...
let (module, _type_environment) = lower_ast(build_ast_with_registered_types(vec![can_succeed_function, start_function], entry_path), &mut string_table, &mut path_fork)
...
#[test]
fn statement_catch_handler_lowering_builds_explicit_result_branching() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let can_fail_name = super::symbol("can_fail", &mut path_fork, &mut string_table);
let location = None;
...
#[test]
fn multi_bind_lowering_projects_tuple_slots_from_single_rhs_call() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let pair_name = super::symbol("pair", &mut path_fork, &mut string_table);
let location = None;
...
let left_id = path_fork.try_intern_child(start_name, string_table.intern("left")).expect("test path fits");
let right_id = path_fork.try_intern_child(start_name, string_table.intern("right")).expect("test path fits");
...
let (module, _type_environment) = lower_ast(build_ast_with_registered_types(vec![pair_function, start_function], entry_path), &mut string_table, &mut path_fork)
