...
#[test]
fn classifies_entry_start_and_normal_functions() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();

let entry_path = super::symbol("main.moth", &mut path_fork, &mut string_table);
let entry_start = path_fork.try_intern_child(entry_path, string_table.intern(IMPLICIT_START_FUNC_NAME)).expect("test path fits");
let normal_fn = path_fork.try_intern_child(entry_path, string_table.intern("helper")).expect("test path fits");
...
#[test]
fn lowers_exact_stable_origin_to_local_function_id() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
...
#[test]
fn rejects_duplicate_stable_origins_before_lookup_insertion() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let first_path = super::symbol("first", &mut path_fork, &mut string_table);
let second_path = super::symbol("second", &mut path_fork, &mut string_table);
let stable_module_origin = StableModuleOriginIdentity::from_portable_path(
    StablePackageIdentity::project_local("test-project"),
    "shapes".to_owned(),
    ModuleRootRole::Normal,
...
#[test]
fn rejects_unused_concrete_origin_seed() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();

let entry_path = super::symbol("main.moth", &mut path_fork, &mut string_table);
let entry_start = path_fork.try_intern_child(entry_path, string_table.intern(IMPLICIT_START_FUNC_NAME)).expect("test path fits");
let normal_fn = path_fork.try_intern_child(entry_path, string_table.intern("helper")).expect("test path fits");
let orphan_path = path_fork.try_intern_child(entry_path, string_table.intern("orphan")).expect("test path fits");
let stable_module_origin = StableModuleOriginIdentity::from_portable_path(
    StablePackageIdentity::project_local("test-project"),
    "shapes".to_owned(),
    ModuleRootRole::Normal,
...
#[test]
fn hir_validation_rejects_two_origins_for_one_local_function() { let mut path_fork = super::PathInternerFork::empty(); use crate::compiler_frontend::hir::hir_builder::validate_module_for_tests;
