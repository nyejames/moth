...
let (module, _type_environment) =
    lower_ast(ast, &mut string_table, &mut path_fork).expect("HIR lowering should preserve source metadata");

let count_local = local_named(&module, &path_fork, &string_table, "count");
...
let ast = build_ast_with_registered_types(vec![start_function], entry_path);
let (module, _type_environment) =
    lower_ast(ast, &mut string_table, &mut path_fork).expect("HIR lowering should preserve parameter metadata");
...
#[test]
fn reactive_template_dependency_metadata_is_bound_to_hir_value() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let count_path = super::symbol("count", &mut path_fork, &mut string_table);
...
let ast = build_ast_with_registered_types(vec![start_function], entry_path);
let (module, _type_environment) =
    lower_ast(ast, &mut string_table, &mut path_fork).expect("HIR lowering should preserve template metadata");
...
#[test]
fn reachability_records_reactive_runtime_fragment_and_external_sinks() { let mut path_fork = super::PathInternerFork::empty(); let mut string_table = StringTable::new();
let (entry_path, start_name) = super::entry_path_and_start_name(&mut path_fork, &mut string_table);
let count_path = super::symbol("count", &mut path_fork, &mut string_table);
let view_path = super::symbol("view", &mut path_fork, &mut string_table);
let count_source = reactive_source(count_path.clone(), ReactiveSourceKind::Declaration);
...
let ast = build_ast_with_registered_types(vec![start_function], entry_path);
let (module, _type_environment) =
    lower_ast(ast, &mut string_table, &mut path_fork).expect("HIR lowering should preserve sink metadata");
