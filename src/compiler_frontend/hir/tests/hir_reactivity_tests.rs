...
fn reactive_declaration_metadata_is_bound_to_local() {
    ...
    let (module, _type_environment) = lower_ast(ast, &mut string_table, &mut path_fork)
        .expect("HIR lowering should preserve source metadata");
...
fn reactive_parameter_metadata_is_bound_to_function_param() {
    ...
    parameter.value.reactive_source =
        Some(reactive_source(count_path, ReactiveSourceKind::Parameter));
    ...
    let ast = build_ast_with_registered_types(vec![start_function], entry_path);
    let (module, _type_environment) = lower_ast(ast, &mut string_table, &mut path_fork)
        .expect("HIR lowering should preserve parameter metadata");
...
fn reactive_template_dependency_metadata_is_bound_to_hir_value() {
    ...
    let ast = build_ast_with_registered_types(vec![start_function], entry_path);
    let (module, _type_environment) = lower_ast(ast, &mut string_table, &mut path_fork)
        .expect("HIR lowering should preserve template metadata");
...
fn reachability_records_reactive_runtime_fragment_and_external_sinks() {
    ...
    let ast = build_ast_with_registered_types(vec![start_function], entry_path);
    let (module, _type_environment) = lower_ast(ast, &mut string_table, &mut path_fork)
        .expect("HIR lowering should preserve sink metadata");
