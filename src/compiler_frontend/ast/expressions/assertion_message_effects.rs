...
//!
//! WHAT: classifies control-flow effects reachable from an assertion message across the
//!        resolved AST, finalized template IR views, and owned runtime-template handoffs.
...
//! owned-handoff walkers for representations that have already crossed those boundaries. It owns
//! the effect classification only; parsing, type validation, HIR lowering, and runtime execution
//! remain in their respective stages.
//!
//! The traversal is scoped to the current enclosing function. Nested function bodies are opaque,
//! and `break` / `continue` are local to the loop that owns them. Deliberately excluded are plain
//! values, ordinary call control flow, and runtime-template nodes without an expression payload.
...
fn classify_nodes(
    ...
    for node in nodes {
        let effect =
            match &node.kind {
                NodeKind::Return(_) => {
                    Some(EnclosingExitEffect::FunctionReturn(node.location.clone()))
                }
                ...
                NodeKind::Break | NodeKind::Continue if state.loop_depth > 0 => None,
                // A valid AST cannot contain loop control outside an owning loop. Keep malformed
                // input non-escaping here; statement validation owns the user-facing rejection.
