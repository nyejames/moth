...
fn template_with_subscription(store: &mut TemplateIrStore, source: ReactiveSource) -> Expression {
    template_expression_from_tir(
        store,
        reference_expression(source.path, DataType::Int, builtin_type_ids::INT)
...
fn template_expression_from_tir(
    ...
    Expression::template(
        template_with_reference(composed_reference(template_id), span),
...
fn propagates_metadata_from_a_template() {
    ...
    let expression = template_with_subscription(
        &mut store,
        reactive_source(source_path, ReactiveSourceKind::Declaration),
    );
...
fn rebases_reactive_parameter_subscription_to_call_argument_source() {
    ...
    let mut ast = vec![
        function_node(function_path, signature, body, None),
        node(
...
fn substitutes_string_parameter_template_value_from_call_argument() {
    ...
    parameter.value.reactive_template = Some(
        ReactiveTemplateMetadata::from_template_value_parameter(parameter_path, None),
    );
    ...
    let body = vec![node(NodeKind::Return(vec![wrapper_template]), None)];
...
fn references_use_metadata_computed_for_prior_declarations() {
    ...
    let mut ast = vec![
        function_node(function_path, signature, body, None),
        node(NodeKind::VariableDeclaration(declaration), None),
        node(
...
fn annotates_fallback_body_tir_root_metadata() {
    ...
    let fallback_expression = template_with_subscription(
        &mut store,
        reactive_source(fallback_path, ReactiveSourceKind::Declaration),
    );
...
fn sink_operand_expressions_keep_reactive_template_metadata() {
    ...
    let mut ast = vec![
        node(NodeKind::PushStartRuntimeFragment(fragment_template), None),
        node(
