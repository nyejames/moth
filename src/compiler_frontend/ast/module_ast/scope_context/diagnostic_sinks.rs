...
impl ScopeContext {
    ...
    pub fn add_var(
        ...
        if let Some(visible_declarations) = self.visible_declaration_ids.as_mut() {
            Arc::make_mut(visible_declarations).insert(declaration.id);
        }
        self.arena
            .borrow_mut()
            .frame_mut(self.current_frame_id)
            .add_var(declaration, binding_span, path_fork);
    ...
    pub(crate) fn add_compile_time_var(
        ...
        self.arena
            .borrow_mut()
            .frame_mut(self.current_frame_id)
            .add_compile_time_var(declaration, binding_span, path_fork);
        increment_ast_counter(AstCounter::ScopeLocalDeclarationsInserted);
    }
