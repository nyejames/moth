...
fn docs_style_table_and_data_declarations(
    ...
    let mut header_row_file_tokens = template_tokens_from_source("[$children([:\n            <th style=\"border: 1px solid; padding: 0.5em; text-align: left;\">[$slot]</th>\n        ]):[$slot]]",
    string_table,
    ...
    let mut header_row_stream = AstCursor::from_file_tokens(&mut header_row_file_tokens)
        .expect("test token stream must expose an AST cursor");
    let header_row = Template::new(&mut header_row_stream, header_row_path, &header_row_context, vec![], string_table, path_fork)
    ...
    let mut table_stream = AstCursor::from_file_tokens(&mut table_file_tokens)
        .expect("test token stream must expose an AST cursor");
    let table = Template::new(&mut table_stream, table_path, &table_context, vec![], string_table, path_fork)
        .expect("docs-style table wrapper should parse");

    let mut data_file_tokens = template_tokens_from_source("[$children([: <td style=\"padding: 0.2em 0.5em;\">[$slot]</td>]):\n    [$slot]\n]",
    string_table,
    span_builder, path_fork);
    ...
    let mut data_stream = AstCursor::from_file_tokens(&mut data_file_tokens)
        .expect("test token stream must expose an AST cursor");
    let data = Template::new(&mut data_stream, data_path, &data_context, vec![], string_table, path_fork)
...
fn child_wrapper_composition_marks_template_tir_reference_composed() {
    ...
    let mut path_fork = PathInternerFork::empty();
    let mut span_builder = ExtendedSpanBuilder::new();
    let mut file_tokens = template_tokens_from_source("[$children([:<b>[$slot]</b>]): hello [:child] ]",
    &mut string_table,
