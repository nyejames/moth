...
fn docs_style_table_and_data_declarations(
    ...
    let canonical_range = canonical_owner
        .full_range()
        .expect("test token stream must expose canonical source range");
    let mut header_row_stream = AstCursor::from_source_tokens(
        &canonical_owner,
        header_row_file_tokens.canonical_os_path.clone(),
        canonical_range,
    )
    .expect("test token stream must expose an AST cursor");
    header_row_stream
        .set_position(header_row_file_tokens.index)
        .expect("test token stream position must remain in canonical range");
    ...
    let canonical_range = canonical_owner
        .full_range()
        .expect("test token stream must expose canonical source range");
    let mut table_stream = AstCursor::from_source_tokens(
        &canonical_owner,
        table_file_tokens.canonical_os_path.clone(),
        canonical_range,
    )
    .expect("test token stream must expose an AST cursor");
    table_stream
        .set_position(table_file_tokens.index)
        .expect("test token stream position must remain in canonical range");
    ...
    let canonical_range = canonical_owner
        .full_range()
        .expect("test token stream must expose canonical source range");
    let mut data_stream = AstCursor::from_source_tokens(
        &canonical_owner,
        data_file_tokens.canonical_os_path.clone(),
        canonical_range,
    )
    .expect("test token stream must expose an AST cursor");
    data_stream
        .set_position(data_file_tokens.index)
        .expect("test token stream position must remain in canonical range");
