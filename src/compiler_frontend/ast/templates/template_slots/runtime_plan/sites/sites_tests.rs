...
fn versioned_wrapper_keeps_keyed_wrapper_context_on_nested_child() {
    ...
    let wrapper_id = {
        ...
        let root = builder.push_sequence_node(vec![nested_child, slot], empty_location());
        builder.finish_template(
            root,
            Style::default(),
            TemplateType::String,
...
fn wrapper_fill_injects_through_branch_fallback_loop_and_child() {
    ...
    let mut builder = TemplateIrBuilder::new(&mut store);
    let then_slot = builder.push_slot_node(SlotKey::Default, empty_location());
    let fallback_slot = builder.push_slot_node(SlotKey::Default, empty_location());
    let selector_site = builder.store.next_expression_site_id();
    let branch_root = builder.push_branch_chain_node(
        vec![TemplateIrBranch::new(
            TemplateBranchSelector::Bool(bool_expression(true)),
    ...
    let loop_slot = builder.push_slot_node(SlotKey::Default, empty_location());
    let aggregate_slot = builder.push_slot_node(SlotKey::Default, empty_location());
    ...
    let nested_slot = builder.push_slot_node(SlotKey::Default, empty_location());
